...
from .. import CONF_KEYPAD_ID, SwitchArray, switch_array_ns

DEPENDENCIES = ["switch_array"]

SwitchArrayBinarySensor = switch_array_ns.class_(
    "SwitchArrayBinarySensor", binary_sensor.BinarySensor
)
...
CONFIG_SCHEMA = cv.All(
    binary_sensor.binary_sensor_schema(SwitchArrayBinarySensor).extend(
        {
            cv.GenerateID(CONF_KEYPAD_ID): cv.use_id(SwitchArray),
...
async def to_code(config):
    ...
    switch_array = await cg.get_variable(config[CONF_KEYPAD_ID])
    cg.add(switch_array.register_listener(var))
